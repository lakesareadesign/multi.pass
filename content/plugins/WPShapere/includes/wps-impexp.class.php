...
if (!class_exists('WPS_IMP_EXP')) {
    ...
    class WPS_IMP_EXP extends WPSHAPERE
    {
        ...
        function add_impexp_menu() {
            add_submenu_page( WPSHAPERE_MENU_SLUG, __('Import and Export Settings', 'wps'), __('Import-Export Settings', 'wps'), 'manage_options', 'wps_impexp_settings', array($this, 'wps_impexp_settings_page') );
        }
        ...
        <?php
            if(isset($_GET['page']) && $_GET['page'] == 'wps_impexp_settings' && isset($_GET['status']) && $_GET['status'] == 'updated')
            {
                ...
                <div class="updated top">
                    <p><strong><?php echo __('Settings Imported!', 'wps'); ?></strong></p>
                </div>
        <?php
            }
            elseif(isset($_GET['page']) && $_GET['page'] == 'wps_impexp_settings' && isset($_GET['status']) && $_GET['status'] == 'dataerror')
            {
                ?>
                <div class="updated top">
                    <p><strong><?php echo __('You are importing empty data or wrong data format.', 'wps'); ?></strong></p>
                </div>
        <?php
            }

            ?>
                <h3><?php echo __('Reset to default', 'wps'); ?></h3>
                <span><?php echo __('By resetting all settings will be deleted!', 'wps'); ?></span>
                ...
                <h3><?php echo __('Export Settings', 'wps'); ?></h3>
                <div style="padding: 15px 0">
                <span><?php echo __('Save the below contents to a text file.', 'wps'); ?></span>
                <textarea class="widefat" rows="10" ><?php echo $this->wps_get_settings(); ?></textarea>
                </div>

                <h3><?php echo __('Import Settings', 'wps'); ?></h3>
                <div style="padding:15px 0">
                <form name="wps_import_settings_form" method="post" action="">
                        ...
                        <input class="button button-primary button-hero" type="submit" value="<?php echo __('Import Settings', 'wps'); ?>" />
