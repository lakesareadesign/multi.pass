...
WDP.showMessage = function(action) {
	...
	switch (action) {
		...
		case "message":
			...
			if (!text || !text.length) {
				me.msg.find(".extra-text").html("");
				me.msg.find(".default-text").show();
			} else {
				me.msg.find(".extra-text").html(text);
...
WDP.showSuccess = function(action) {
	WDP.showMessage("type", "success")
		.showMessage("delay", 3000)
		.showMessage(action, arguments[1]);

	if ("message" === action) {
		WDP.showMessage("show");
	}
