...
class WDS_Settings_Settings extends WDS_Settings_Admin {
	...
	private function _validate_and_save_extra_options ($input) {
		...
		$sitemaps = WDS_Settings::get_component_options(WDS_Settings::COMP_SITEMAP);
		...
		if ($sitemaps_updated) {
			WDS_Settings::update_component_options(WDS_Settings::COMP_SITEMAP, $sitemaps);
		}
	}
	...
	public function init () {
		...
		if (!class_exists('WDS_Controller_IO')) {
			require_once(WDS_PLUGIN_DIR . '/core/class_wds_controller_io.php');
		}
		WDS_Controller_IO::serve();
	...
	function activate_component()
	{
		if (isset($_POST['wds-activate-component'])) {
			...
			$component = $_POST['wds-activate-component'];
	...
	public function options_page () {
		...
		$arguments['slugs'] = array(
			...
			WDS_Settings::TAB_CHECKUP => __( 'SEO Checkup', 'wds' ),
			WDS_Settings::TAB_SITEMAP => __( 'Sitemaps', 'wds' ),
			WDS_Settings::TAB_AUTOLINKS => __( 'Advanced Tools', 'wds' ),
			WDS_Settings::TAB_SOCIAL => __( 'Social', 'wds' ),
		...
		$arguments['wds_sitewide_mode'] = wds_is_switch_active('WDS_SITEWIDE') || (bool)get_site_option('wds_sitewide_mode');

		$wds_options = WDS_Settings::get_options();
		$sitemap_settings = WDS_Sitemap_Settings::get_instance();
	...
	private function get_old_user_role_options()
	{
		...
		foreach ($option_keys as $option_key) {
			if ($option_value = wds_get_array_value($old_options, $option_key)) {
