<?php
	...
	$og_enabled = wds_get_array_value($options, 'og-enable');
	$twitter_card_enabled = wds_get_array_value($options, 'twitter-card-enable');
	$twitter_card_type = wds_get_array_value($options, 'twitter-card-type');
	$twitter_card_status_text = $twitter_card_type == WDS_Twitter_Printer::CARD_IMAGE ? esc_html__('Summary Card with Large Image', 'wds') : esc_html__('Summary Card', 'wds');
	$pinterest_verification_status = wds_get_array_value($options, 'pinterest-verification-status');
	$pinterest_tag = wds_get_array_value($options, 'pinterest-verify');

	$social_page_url = WDS_Settings_Admin::admin_url(WDS_Settings::TAB_SOCIAL);
	$social_option_name = WDS_Settings::TAB_SOCIAL . '_options';
	$settings_option_name = WDS_Settings::TAB_SETTINGS . '_options';
	$social_enabled = wds_get_array_value($options, 'social');
?>
<section id="<?php echo WDS_Settings_Dashboard::BOX_SOCIAL; ?>" class="dev-box">
	<div class="box-title">
