<?php
	$page_url = WDS_Settings_Admin::admin_url(WDS_Settings::TAB_SITEMAP);
	$options = $_view['options'];
	$sitemap_enabled = wds_get_array_value($options, 'sitemap');
...
?>
<section id="<?php echo WDS_Settings_Dashboard::BOX_SITEMAP; ?>"
		 class="dev-box"
		 data-dependent="<?php echo WDS_Settings_Dashboard::BOX_TOP_STATS; ?>">
			...
			<?php if ($sitemap_enabled): ?>
				...
				$this->_render('notice', array(
					...
					'message' => sprintf(
						...
						sprintf('<a href="%s">/sitemap.xml</a>', esc_attr(wds_get_sitemap_url()))
					)
				));
				?>
