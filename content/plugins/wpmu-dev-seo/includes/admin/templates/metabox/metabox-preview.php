<?php
	...
	// Title
	$title = empty($title) ? wds_get_value('title') : $title;
	if (empty($title) && $post) {
	...
	if (is_object($post)) $title = wds_replace_vars($title, (array)$post);

	// Description
	$description = empty($description) ? wds_get_value('metadesc') : $description;
	...
	if (is_object($post)) $description = wds_replace_vars($description, (array)$post);
