<?php
	...
	$access_id = empty($wds_options['access-id']) ? '' : $wds_options['access-id'];
	$secret_key = empty($wds_options['secret-key']) ? '' : $wds_options['secret-key'];
?>

<?php if (empty($access_id) || empty($secret_key)): ?>
	<div class="wds-disabled-component">
		<p>
			<img src="<?php echo WDS_PLUGIN_URL; ?>/images/<?php echo esc_attr('moz-disabled.png'); ?>" alt="<?php esc_html_e('', 'wds'); ?>" class="wds-disabled-image"/>
		</p>
		<p><?php _e('Moz provide reports that tell you how your site stacks up against the competition with all of the important SEO measurement tools - ranking, links, and much more.', 'wds'); ?></p>
	</div>
...
<?php else: ?>
	...
	<?php WDS_Seomoz_Dashboard_Widget::widget(); ?>
<?php endif; ?>