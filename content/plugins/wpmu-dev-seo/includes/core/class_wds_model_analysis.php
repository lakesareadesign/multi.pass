...
class WDS_Model_Analysis extends WDS_Model {
	...
	public function get_type () { return 'analysis'; }
	...
	public function get_post_data ($data_type) {
		...
		$data = wds_get_value($data_type, $this->_post_id);
	...
	public function get_readability_threshold () {
		...
		return WDS_String::READABILITY_FLESCH === WDS_String::get_readability_strategy()
			? $threshold_flesch
			: $threshold_kincaid
		;
	}
	...
	public function is_readable () {
		...
		if (WDS_Checks::is_readability_ignored($this->_post_id)) return true;
	...
	public function update_analysis_data () {
		if (!class_exists('WDS_Checks')) require_once(WDS_PLUGIN_DIR . '/core/class_wds_checks.php');
		$checks = WDS_Checks::apply($this->_post_id);
	...
	public function update_readability_data () {
		...
		$str = WDS_Html::plaintext(wds_get_post_or_latest_revision($this->_post_id)->post_content);
		...
		;
		$result = 0;
		$raw_result = WDS_String::readability_score($str);

		if (false === $raw_result) $error = !empty($error) ? $error : __('Error calculating readability', 'wds');
		else $result = $this->normalize_readability_score($raw_result);

		$is_readable = WDS_String::get_readability_strategy() === WDS_String::READABILITY_FLESCH
	 ...
	 *
	 * @param $readability_score float Score for which range needs to be found.
	 * @param $readability_ignored bool Whether readability is marked as ignored by the user.
	...
	public function get_kincaid_readability_state ($readability_score, $readability_ignored) {
		...
		if (WDS_String::get_readability_strategy() == WDS_String::READABILITY_KINCAID) {
			if ($readability_score == 0 || $readability_ignored) {
				$readability_state = 'invalid';
			} else if ($readability_score < 60) {
				$readability_state = 'error';
			} else if ($readability_score < 70) {
				$readability_state = 'warning';
			} else if ($readability_score <= 100) {
	 ...
	 *
	 * @param $level string Readability level key.
	...
	function get_overall_seo_analysis()
	{
		...
		foreach ($post_types as $type) {
			...
			foreach ($posts as $post) {
				...
				$has_seo_data = $this->has_post_data(WDS_Model_Analysis::DATA_ANALYSIS);
				if ($has_seo_data) {

					$checked++;
					$checked_in_type++;

					$seo_data = $this->get_post_data(WDS_Model_Analysis::DATA_ANALYSIS);
					$percentage = intval(wds_get_array_value($seo_data, 'percentage'));
	...
	function get_overall_readability_analysis()
	{
		...
		foreach ($post_types as $type) {
			...
			foreach ($posts as $post) {
				 ...
				 */
				$this->set_post_id($post->ID);
				$data_available = $this->has_post_data(WDS_Model_Analysis::DATA_READABILITY);
				if ($data_available) {

					$data = $this->get_post_data(WDS_Model_Analysis::DATA_READABILITY);
					$post_score = intval(wds_get_array_value($data, 'score'));

					$readability_state = $this->get_kincaid_readability_state($post_score, false);

					$type_overview[ $readability_state ] = intval(wds_get_array_value($type_overview, $readability_state)) + 1;

					if ($readability_state == 'warning' || $readability_state == 'success') {
						$passed++;
					}

					if ($readability_state !== 'invalid') {
						$checked++;
					}
				}
			}
