...
class WDS_Check_Title_Length extends WDS_Check_Post_Abstract {
	...
	public function apply () {
		...
		if (!is_object($post) || empty($post->ID)) {
		...
		} else if (wp_is_post_revision($post->ID) && !empty($post->post_title)) {
			...
			$parent_subject = WDS_OnPage::get()->get_title($post->post_title);
		...
		$this->_length = WDS_String::len($subject);
	...
	public function apply_html () {
		...
		$this->_length = WDS_String::len($title);
	...
	public function get_recommendation()
	{
		...
		} else if ($title_length > $this->get_max()) {
		...
		} else if ($title_length < $this->get_min()) {
	...
	public function get_min () { return 50; }
	public function get_max () { return 70; }
}