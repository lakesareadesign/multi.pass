...
if (!class_exists('WDS_Model_Analysis')) require_once(WDS_PLUGIN_DIR . '/core/class_wds_model_analysis.php');
if (!class_exists('WDS_Renderable')) require_once(WDS_PLUGIN_DIR . '/core/class_wds_renderable.php');

class WDS_Controller_Analysis extends WDS_Renderable {
	 ...
	 *
	 * @return WDS_Controller_Analysis instance
	 */
	...
	public function apply_analysis_post_list_filter ($query) {
		...
		if (isset($_GET['wds_analysis_threshold'])) {
			...
			$meta_queries[] = array(
				'key' => WDS_Model_Analysis::META_KEY_ANALYSIS,
		...
		if (isset($_GET['wds_readability_threshold'])) {
			...
			$meta_queries[] = array(
				'key' => WDS_Model_Analysis::META_KEY_READABILITY,
	...
	public function inject_script_dependencies ($hook) {
		...
		wp_enqueue_script(
			...
			WDS_PLUGIN_URL . 'js/wds-admin-analysis-postlist.js',
		...
		wp_enqueue_style(
			...
			WDS_PLUGIN_URL . 'css/wds-admin-analysis-postlist.css',
	...
	{
		if (!class_exists('WDS_Checks')) {
			require_once(WDS_PLUGIN_DIR . '/core/class_wds_checks.php');
		}

		$model = new WDS_Model_Analysis($post->ID);
		if (in_array(get_post_status($post), array('draft', 'auto-draft'))) {
			$result = WDS_Checks::apply($post->ID);
			...
			$focus_keywords = wds_get_value('focus-keywords');
			$focus_keywords_available = !empty($focus_keywords);
			$seo_data = $model->get_post_data(WDS_Model_Analysis::DATA_ANALYSIS);
		...
		$readability_data = $model->get_post_data(WDS_Model_Analysis::DATA_READABILITY);
		$readability_score = wds_get_array_value($readability_data, 'score');
		$readability_score = intval(ceil($readability_score));
		$readability_ignored = WDS_Checks::is_readability_ignored($post->ID);
		...
		<div class="wds-post-box-fields">
			...
			<?php if (WDS_Settings::get_setting('analysis-readability')): ?>
	...
	public function add_seo_analysis_metabox_content ($post) {
		if (!WDS_Settings::get_setting('analysis-seo')) return false;

		if (!class_exists('WDS_Checks')) require_once(WDS_PLUGIN_DIR . '/core/class_wds_checks.php');
		/**
		 * @var WDS_Checks $result
		 */
		$result = WDS_Checks::apply($post->ID);
		$checks = $result->get_applied_checks();
		$focus_keywords_available = isset($checks['focus']['status']) ? $checks['focus']['status'] : false;
	...
	public function add_readability_analysis_metabox_content ($post) {
		if (!WDS_Settings::get_setting('analysis-readability')) return false;
	...
	public function add_analysis_columns ($columns) {
		if (WDS_Settings::get_setting('analysis-seo')) {
	...
	public function get_post_analysis_result_markup ($post_id) {
		$model = new WDS_Model_Analysis($post_id);
		$result = array(
			'seo' => '',
			'readability' => '',
		);
		...
		if (!$model->has_post_data(WDS_Model_Analysis::DATA_ANALYSIS)) {
			$result['seo'] = $na;
		} else {
			$data = $model->get_post_data(WDS_Model_Analysis::DATA_ANALYSIS);
			if (empty($data['errors'])) {
				...
				$percentage = !empty($data['percentage'])
					...
					: __('Errors:', 'wds')
				;
				$result['seo'] = '<div class="wds-analysis wds-status-error" title="' . esc_attr($percentage) . ' "><span>' . $percentage . '</span></div>';
		...
		if (!$model->has_post_data(WDS_Model_Analysis::DATA_READABILITY)) {
			$result['readability'] = $na;
		} else {
			if (!class_exists('WDS_String')) require_once(WDS_PLUGIN_DIR . '/core/class_wds_string.php');
			if (!class_exists('WDS_Checks')) require_once(WDS_PLUGIN_DIR . '/core/class_wds_checks.php');

			$data = $model->get_post_data(WDS_Model_Analysis::DATA_READABILITY);
			$readability_score = intval(ceil(wds_get_array_value($data, 'score')));
			$readability_ignored = WDS_Checks::is_readability_ignored($post_id);
			$readability_state = $model->get_kincaid_readability_state($readability_score, $readability_ignored);
			$readability_class = sprintf(
				'wds-status-%s',
				$readability_state
	...
	public function json_set_ignore_check () {
		...
		if (!class_exists('WDS_Checks')) require_once(WDS_PLUGIN_DIR . '/core/class_wds_checks.php');
		WDS_Checks::add_ignored_check($data['post_id'], sanitize_text_field($data['check_id']));

		$model = new WDS_Model_Analysis($data['post_id']);
	...
	public function json_unset_ignore_check () {
		...
		if (!class_exists('WDS_Checks')) require_once(WDS_PLUGIN_DIR . '/core/class_wds_checks.php');
		WDS_Checks::remove_ignored_check($data['post_id'], sanitize_text_field($data['check_id']));
	...
	public function json_get_post_editor_analysis () {
		 ...
		 */
		$post = wds_get_post_or_latest_revision($data['post_id']);
		$out = array();
		ob_start();
	...
	public function json_get_post_analysis_data () {
		...
		$this->maybe_analyze_post($data['post_id']);
		$model = new WDS_Model_Analysis($data['post_id']);

		return wp_send_json_success(array(
			'analysis' => $model->get_post_data(WDS_Model_Analysis::DATA_ANALYSIS),
			'readability' => $model->get_post_data(WDS_Model_Analysis::DATA_READABILITY),
	...
	public function json_get_post_analysis_markup () {
		...
		$this->maybe_analyze_post($data['post_id']);
		$result = $this->get_post_analysis_result_markup($data['post_id']);
	...
	public function json_get_post_analysis_recheck () {
		...
		$this->analyze_post($data['post_id']);
		$result = $this->get_post_analysis_result_markup($data['post_id']);
