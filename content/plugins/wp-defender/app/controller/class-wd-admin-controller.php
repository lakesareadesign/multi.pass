...
class WD_Admin_Controller extends WD_Controller {
	...
	public function __construct() {
		...
		$this->add_filter( 'wdp_register_hub_action', 'add_my_endpoint' );
	}

	public function add_my_endpoint( $actions ) {
		$actions['defender_new_scan']      = array( &$this, 'new_scan' );
		$actions['defender_schedule_scan'] = array( &$ths, 'schedule_scan' );
	...
	public function new_scan( $params, $action ) {
		$ret = WD_Scan_Api::create_scan_record();
		if ( is_wp_error( $ret ) ) {
	...
	public function schedule_scan( $params, $action ) {
		...
		$day       = $params['day'];
		$time      = $params['time'];
		if ( ! in_array( $frequency, WD_Scan_Api::get_frequently() ) || ! in_array( $day, WD_Scan_Api::get_days_of_week() ) || ! in_array( $time, WD_Scan_Api::get_times() ) ) {
			wp_send_json_error();
		}
