...
class CoursePress_Template_Communication {
	...
	public static function render_discussion() {
		...
		$content .= sprintf(
			...
			esc_html__( 'Discussion', 'cp' ),
			esc_html_x( ': ', 'separator between "Discussion" text and title', 'cp' ),
			esc_html( $title )
		);
		...
		if ( get_current_user_id() == (int) $author ) {
			...
			$content .= sprintf( '<a href="%s">%s</a>', esc_url( $edit_discussion_link ), esc_html__( 'Edit', 'cp' ) );
			$content .= '</div>';
		}
		$content .= '</div>';
