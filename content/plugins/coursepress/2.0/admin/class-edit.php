	...
	public static function step_2() {
		...
		$content .= '
				<div class="wide">
						...
						<label class="checkbox">
							<input type="radio" name="meta_course_view" ' . CoursePress_Helper_Utility::checked( CoursePress_Data_Course::get_setting( $course_id, 'course_view', 'normal' ), 'focus' ) . ' value="focus">' . esc_html__( 'Focus: Focus on one item at a time', 'cp' ) . '
						</label>
						<label class="checkbox">
				...
				<div class="wide">
					...
					<div class="course-structure">
						...
						<label class="checkbox">
							...
							<span>' . esc_html__( 'Show units without modules', 'cp' ) . '</span>
							<p class="description">'.esc_html__( 'By default unit without modules is not displayed, even if it is selected below.', 'cp' ).'</p>
						</label>
...
<li><label><input type="radio" name="invite_instructor_type" value="facilitator" /> ' . __( 'Facilitator', 'cp' ) . '</label></li>
	...
	public static function step_6() {
		...
		if ( ! $payment_supported && ! $disable_payment ) {
			...
			if ( CP_IS_PREMIUM ) {
				$version_message = sprintf( '<p>%s</p>', __( 'The full version of MarketPress has been bundled with CoursePress Pro.', 'cp' ) );
			} else {
				$version_message = sprintf( '<p>%s</p>', __( 'You can use the free or premium version of MarketPress to sell your courses.', 'cp' ) );
			}
	...
	public static function step_7() {
		...
		$one = array(
			...
			'description' => __( 'Use this field to override general course certificate setting.', 'cp' ),
			'label' => __( 'Override course certificate.', 'cp' ),
			'default' => false,
		);
		$content .= CoursePress_Helper_UI::course_edit_checkbox( $one, $course_id );
		...
		$content .= '<tr><td><label>' . __( 'Content margin', 'cp' ) . '</label></td><td>';
		$content .= __( 'Top', 'cp' ) . ': <input type="number" class="small-text" name="meta_cert_margin[top]" value="'. esc_attr( $margin_top ) . '" />';
		$content .= __( 'Left', 'cp' ) . ': <input type="number" class="small-text" name="meta_cert_margin[left]" value="'. esc_attr( $margin_left ) . '" />';
		$content .= __( 'Right', 'cp' ) . ': <input type="number" class="small-text" name="meta_cert_margin[right]" value="'. esc_attr( $margin_right ) . '" />';
		$content .= '</td></tr>';
