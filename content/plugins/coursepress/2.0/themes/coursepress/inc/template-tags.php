		...
		<nav class="navigation paging-navigation" role="navigation">
			...
			<div class="nav-links">
				...
				<?php if ( get_next_posts_link() ) : ?>
					...
					next_posts_link(
						sprintf(
							__( '%s&larr;%s Older posts', 'coursepress' ),
							'<span class="meta-nav">',
							'</span>'
						)
					);
				...
				<?php if ( get_previous_posts_link() ) : ?>
					...
					previous_posts_link(
						sprintf(
							__( 'Newer posts %s&rarr;%s', 'coursepress' ),
							'<span class="meta-nav">',
							'</span>'
						)
					);
...
if ( ! function_exists( 'coursepress_post_nav' ) ) :
	...
	function coursepress_post_nav() {
		...
		<nav class="navigation post-navigation" role="navigation">
			...
			<?php esc_html_e( 'Post navigation', 'coursepress' ); ?>
			</h1>
			<div class="nav-links">
				<?php
				previous_post_link(
					'%link',
					sprintf(
						_x( '%s&larr;%s %%title', 'Previous post link', 'coursepress' ),
						'<span class="meta-nav">',
						'</span>'
					)
				);
				next_post_link(
					'%link',
					sprintf(
						_x( '%%title %s&rarr;%s', 'Next post link', 'coursepress' ),
						'<span class="meta-nav">',
						'</span>'
					)
				);
		...
		if ( 'pingback' == $comment->comment_type || 'trackback' == $comment->comment_type ) : ?>
			...
			<li id="comment-<?php comment_ID(); ?>" <?php comment_class(); ?>>
				<div class="comment-body">
					<?php
					esc_html_e( 'Pingback:', 'coursepress' );
					comment_author_link();
					edit_comment_link(
						__( 'Edit', 'coursepress' ),
				...
				<article id="div-comment-<?php comment_ID(); ?>" class="comment-body">
					<footer class="comment-meta">
						<div class="comment-author vcard">
							...
							printf(
								esc_html__( '%s says:', 'coursepress' ) . '</span>',
								sprintf(
						...
						<div class="comment-metadata">
							<a href="<?php echo esc_url( get_comment_link( $comment->comment_ID ) ); ?>">
								<time datetime="<?php comment_time( 'c' ); ?>">
									<?php
									printf(
										_x( '%1$s at %2$s', '1: date, 2: time', 'coursepress' ),
										get_comment_date(),
										get_comment_time()
									);
									?>
						...
						<?php if ( '0' == $comment->comment_approved ) : ?>
							<p class="comment-awaiting-moderation">
								<?php esc_html_e( 'Your comment is awaiting moderation.', 'coursepress' ); ?>
							</p>
						<?php endif; ?>
					</footer><!-- .comment-meta -->
					...
					<div class="comment-content">
						...
						<a href="<?php echo esc_url( get_comment_link( $comment->comment_ID ) ); ?>">
							<time>
								<?php
								printf(
									_x( '%1$s, %2$s ago', '1: author, 2: time of comment', 'coursepress' ),
									comment_author(),
									human_time_diff(
										get_comment_time( 'U' ),
...
if ( ! function_exists( 'coursepress_posted_on' ) ) :
	...
	function coursepress_posted_on() {
		...
		printf(
			__( '<span class="posted-on">Posted on %1$s</span><span class="byline"> by %2$s</span>', 'coursepress' ),
			sprintf(
