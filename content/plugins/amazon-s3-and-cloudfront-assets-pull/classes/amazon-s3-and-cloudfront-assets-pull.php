...
use DeliciousBrains\WP_Offload_S3_Assets_Pull\Addon_Activation_Data;
use DeliciousBrains\WP_Offload_S3_Assets_Pull\Domain_Check;
use DeliciousBrains\WP_Offload_S3_Assets_Pull\Domain_Check_Controller;
use DeliciousBrains\WP_Offload_S3_Assets_Pull\Domain_Check_Response;
...
class Amazon_S3_And_CloudFront_Assets_Pull extends Amazon_S3_And_CloudFront_Pro {
	...
	/* @var Addon_Activation_Data */
	protected $s3_addon_data;

	const SETTINGS_KEY = 'as3cf_assets_pull';
	const SETTINGS_CONSTANT = 'WPOS3_ASSETS_PULL_SETTINGS';
	...
	public function init( $plugin_file_path ) {
		$this->s3_addon_data           = new Addon_Activation_Data( 'assets' );
		...
		add_action( 'admin_notices', array( $this, 'display_s3_addon_deactivated_notice' ) );
		add_action( 'network_admin_notices', array( $this, 'display_s3_addon_deactivated_notice' ) );
		...
		add_action( 'wp_ajax_' . $this->action_key( 'dismiss_s3_addon_deactivated_notice' ), array( $this, 'ajax_dismiss_s3_addon_deactivated_notice' ) );
	...
	protected function send_domain_check_failure( $exception, $check ) {
		...
		$response['more_info']       = $this->domain_check_more_info( $exception );
		$response['last_checked_at'] = self::last_checked_datetime( $result['timestamp'] );
	...
	public function settings_tabs( $tabs ) {
		$tabs['assets_pull'] = _x( 'Assets', 'Show the Assets settings tab', 'amazon-s3-and-cloudfront-assets-pull' );
	...
	public function assets_more_info_link( $hash, $utm_content = '' ) {
		return $this->more_info_link( '/wp-offload-s3/doc/assets-pull-addon-settings/', $utm_content, $hash );
	}
	...
	protected function domain_check_more_info( $message ) {
		...
		return $this->dbrains_url( '/wp-offload-s3/doc/', $more_info_params );
	}
	...
	/**
	 * Handle possible activation conflict with old S3 Assets addon.
	 *
	 * If one addon is being activated, we deactivate the other.
	 * If they are both activated at the same time, we deactivate the old addon.
	 * Otherwise we deactivate the old addon if it is detected.
	 */
	public function handle_activation() {
		if ( ! self::s3_addon_instance() ) {
			return;
		}

		if ( $this->is_just_activated() ) {
			$this->deactivate_s3_assets_addon();
		} elseif ( $this->s3_addon_just_activated() ) {
			$this->deactivate();
		}
	}
	...
	public function display_activated_notice() {
		...
		$message   = '<strong>Assets Pull Addon for WP Offload S3</strong> &mdash; ';
		$message   .= sprintf(
	...
	/**
	 * Whether or not the deactivated notice should be shown for the old Assets addon.
	 *
	 * @return bool
	 */
	public function should_show_s3_addon_deactivated_notice() {
		return current_user_can( 'activate_plugins' ) && $this->s3_addon_data->deactivated_within( 15 * MINUTE_IN_SECONDS );
	}

	/**
	 * Deactivate the deprecated S3 addon if active.
	 */
	public function deactivate_s3_assets_addon() {
		if ( self::s3_addon_instance() ) {
			AS3CF_Utils::deactivate_plugins( self::s3_addon_instance()->get_plugin_file_path() );
		}
	}

	/**
	 * Display the notice indicating the old addon has been deactivated, if necessary.
	 */
	public function display_s3_addon_deactivated_notice() {
		if ( ! self::should_show_s3_addon_deactivated_notice() ) {
			return;
		}

		$notice_id = 'as3cf_assets_s3_deactivated';
		$message   = __(
			'<strong>Old Assets Addon Deactivated</strong> &mdash; ' .
			"The new Assets Pull addon and the old Assets addon can't both be active at the same time. " .
			"Since you just activated the new Assets Pull addon, we've automatically deactivated the old one. " .
			"The old Assets addon has been deprecated and will not be supported in the near future so we encourage you to replace it with the new Assets Pull addon. ",
			'amazon-s3-and-cloudfront-assets-pull'
		);
		$message  .= $this->more_info_link( '/wp-offload-s3/doc/assets-pull-addon/', 's3+assets+deactivated+notice' );
		$message  .= $this->capture_view( 'script/inline-dismiss', array(
			'localize_data' => array(
				'container_selector' => '#' . $notice_id,
				'action'             => $this->action_key( 'dismiss_s3_addon_deactivated_notice' ),
				'nonce'              => $this->create_nonce( 'dismiss_s3_addon_deactivated_notice' ),
			),
		) );
	...
	public function ajax_dismiss_activated_notice() {
		...
		$this->s3_addon_data->update( 'activated_at', null );

		wp_send_json_success();
	}

	/**
	 * Ajax handler to dismiss the S3 addon deactivated notice.
	 */
	public function ajax_dismiss_s3_addon_deactivated_notice() {
		check_ajax_referer( $this->action_key( 'dismiss_s3_addon_deactivated_notice' ) );

		$this->addon_data->update( 'deactivated_at', null );
		$this->s3_addon_data->update( 'deactivated_at', null );

		wp_send_json_success();
	}

	/**
	 * Deactivate this plugin to not conflict with the old S3 Assets addon.
	 */
	private function deactivate() {
		$message = __(
			"<strong>New Assets Pull Addon Deactivated</strong> &mdash; " .
			"The new Assets Pull addon and the old Assets addon can't both be active at the same time. " .
			"Since you just activated the old Assets addon, we've automatically deactivated the new one. " .
			"The old Assets addon has been deprecated and will not be supported in the near future so we encourage you to replace it with the new Assets Pull addon. ",
			'amazon-s3-and-cloudfront-assets-pull'
		) . $this->more_info_link( '/wp-offload-s3/doc/assets-pull-addon/', 'assets+pull+addon+deactivated+notice' );

		$notice_id = 'assets_pull_deactivated_for_legacy';
		$this->notices->undismiss_notice_for_all( $notice_id );
		$this->notices->add_notice( $message, array(
			'custom_id'         => $notice_id,
			'user_capabilities' => array( 'activate_plugins' ),
		) );

		AS3CF_Utils::deactivate_plugins( $this->plugin_file_path );
	}
	...
	protected function is_just_activated() {
		...
		return $this->addon_data->get( 'activated_at' ) >= $this->s3_addon_data->get( 'activated_at' );
	}

	/**
	 * Whether the old Assets addon was recently activated or not.
	 *
	 * @return bool
	 */
	protected function s3_addon_just_activated() {
		if ( ! self::s3_addon_instance() ) {
			return false;
		}

		return $this->s3_addon_data->activated_within( 15 * MINUTE_IN_SECONDS );
	}

	/**
	 * Get the class instance of the old Assets addon, if it has been instantiated.
	 *
	 * @return bool|Amazon_S3_And_CloudFront_Assets
	 */
	public static function s3_addon_instance() {
		if ( isset( $GLOBALS['as3cf_assets'] ) && $GLOBALS['as3cf_assets'] instanceof Amazon_S3_And_CloudFront_Assets ) {
			return $GLOBALS['as3cf_assets'];
		}
