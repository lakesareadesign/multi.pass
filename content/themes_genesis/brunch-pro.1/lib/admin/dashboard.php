<?php
/**
 * The Theme Dashboard.
 *
 * @package   BrunchPro\Functions\Admin
 * @copyright Copyright (c) 2017, Feast Design Co.
 * @license   GPL-2.0+
 * @since     2.2.0
 */

defined( 'WPINC' ) || die;

add_action( 'after_switch_theme', 'brunch_pro_dashboard_setup', 10 );
/**
 * Set up the dashboard options.
 *
 * @since   3.1.0
 * @access  public
 * @return  void
 */
function brunch_pro_dashboard_setup() {
	if ( ! is_network_admin() ) {
		add_option( 'brunch_pro_dashboard_redirect', true, '', 'no' );
	}
}

add_action( 'after_switch_theme', 'brunch_pro_dashboard_redirect', 12 );
/**
 * Add options and fire a redirect when the theme is first activated.
 *
 * @since   3.1.0
 * @access  public
 * @return  void
 */
function brunch_pro_dashboard_redirect() {
	// Bail if we've already been redirected.
	if ( is_network_admin() || ! get_option( 'brunch_pro_dashboard_redirect' ) ) {
		return;
	}

	// Make sure this doesn't go into a redirect loop.
	update_option( 'brunch_pro_dashboard_redirect', false );

	wp_safe_redirect( admin_url( 'admin.php?page=brunch-pro-dashboard' ) );
	exit;
}

add_action( 'switch_theme', 'brunch_pro_dashboard_cleanup', 10 );
/**
 * Remove our redirect option so we will be redirected again on the next activation.
 *
 * @since   3.1.0
 * @access  public
 * @return  void
 */
function brunch_pro_dashboard_cleanup() {
	delete_option( 'brunch_pro_dashboard_redirect' );
}

/**
 * Return the theme's SVG icon.
 *
 * @since  3.1.0
 * @access public
 * @return string A base64 encoded SVG icon for the theme.
 */
function brunch_pro_get_svg_icon() {
	$icon = '';

	return "data:image/svg+xml;base64,{$icon}";
}

add_action( 'admin_menu', 'brunch_pro_dashboard_menu', 0 );
/**
 * Add the theme dashboard to the main WordPress dashboard menu.
 *
 * @since   3.1.0
 * @access  public
 * @return  void
 */
function brunch_pro_dashboard_menu() {
	add_menu_page(
		'Brunch Pro',
		'Brunch Pro',
		'edit_theme_options',
		'brunch-pro-dashboard',
		'brunch_pro_dashboard_page',
		brunch_pro_get_svg_icon(),
		'58.997'
	);
}

/**
 * Include the base template for our dashboard page.
 *
 * @since   3.1.0
 * @access  public
 * @return  void
 */
function brunch_pro_dashboard_page() {
	require_once BRUNCH_PRO_DIR . 'lib/admin/views/dashboard.php';
}

add_action( 'admin_enqueue_scripts', 'brunch_pro_dashboard_scripts', 10 );
/**
 * Load scripts and styles for the dashboard.
 *
 * @since   3.1.0
 * @access  public
 * @param   object $screen The current screen object.
 * @return  void
 */
function brunch_pro_dashboard_scripts( $screen ) {
	if ( 'toplevel_page_brunch-pro-dashboard' === $screen ) {
		wp_enqueue_style(
			'brunch-pro-dashboard',
			BRUNCH_PRO_URI . 'css/dashboard.css',
			array(),
			CHILD_THEME_VERSION
		);
	}
}
