 ...
 * Slush Pro.
 *
 * This defines the custom post meta for use in the Slush Pro Theme.
 *
 * @package Slush_Pro
 * @author  ZigzagPress
 * @license GPL-2.0+
...
class Super_Custom_Post_Meta {
	...
	/**
	 * Constructs a new Super_Custom_Post_Meta object for the given post type.
	 *
	...
	/**
	 * Hooks our meta boxes into WordPress.
	 *
	...
	/**
	 * Adds a field to a meta box. Sorts out what kind of field it is, and calls the appropriate method to output it.
	 *
	...
	/**
	 * Adds a text field to a meta box.
	 *
	...
	if ( !empty(  $field['data-desc'] ) )
		echo SCPT_Markup::tag( 'span', array(
				'id'    => 'scpt_meta_' . $field['meta_key'],
				'class' => 'scpt-meta-description scpt-meta-text label-'), $field['data-desc'] );
	}

	/**
	 * Adds a color field to a meta box.
	 *
	 * @uses SCPT_Markup::tag
	 * @param array $field The field information.
	 * @param array $post_meta The post meta from the database.
	 ...
	 * @author Matthew Boynes
	 */
	public function add_color_field( $field, $post_meta, $html_attributes ) {
		...
		echo SCPT_Markup::tag( 'input', array_merge( array(
				...
				'value' => ( isset( $post_meta[ $field['meta_key'] ] ) ? $post_meta[ $field['meta_key'] ][0] : '' ),
				'name'  => $field['meta_key'],
				'class' => 'scpt-field  scpt-field-color',
				'id'    => 'scpt_meta_' . $field['meta_key'],
			), $html_attributes ) );
	// Added by joe to add field description.
	if ( !empty(  $field['data-desc'] ) )
	...
	/**
	 * Adds a textarea field to a meta box.
	 *
	 * @uses SCPT_Markup::tag
	 * @param array $field The field information.
	 * @param array $post_meta The post meta from the database.
	...
	/**
	 * Adds multiple checkbox fields to a meta box, all with the same name.
	 *
	 * @uses SCPT_Markup::tag
	...
	/**
	 * Adds multiple radio fields to a meta box, all with the same name.
	 *
	 * @uses SCPT_Markup::tag
	 * @uses add_boolean_field
	 * @uses prune_options
	...
	/**
	 * Adds media field to a meta box.
	 *
	 * @uses SCPT_Markup::tag
	 * @param array $field The field information.
	 * @param array $post_meta The post meta from the database.
	...
	public function add_media_field( $field, $post_meta, $html_attributes ) {
		...
		if ( $value ) {
			...
			$preview = '<span class="scpt-image_holder">';
			...
			$preview .= sprintf( '<a href="#" class="button scpt-remove-thumbnail">%s</a>', 'X' );
			$preview .= '</span>';
		} else {
			$preview = '';
		}
		...
		echo SCPT_Markup::tag( 'p', '',
			...
			'href'  => '#',
			'class' => 'button scpt-add-media',
			'style' => ( $value ? 'display:none' : '' ),
			), $html_attributes ), sprintf( __( 'Add Image', 'slush-pro' ), $field['label'] ) )
		);
	...
	/**
	 * Adds multiple media field to a meta box.
	 *
	 * @uses SCPT_Markup::tag
	 * @param array $field The field information.
	 * @param array $post_meta The post meta from the database.
	...
	public function add_multiple_media_field( $field, $post_meta, $html_attributes ) {
		...
		if( $ids ){
			while($i < count($ids)){    
				if ( isset($ids[$i]) ) {
					...
					if ( isset( $attachment->post_mime_type ) && strpos( $attachment->post_mime_type, 'image/' ) === 0 ) {
						$preview .= sprintf( '<div class="zp_image_holder" data-attachment_id="'.$ids[$i].'">');
						$preview .= wp_get_attachment_image( $ids[$i], 'thumbnail', false, array( 'class' => $ids[$i] ) );
						$preview .= sprintf( '<br /><a href="#" class="button scpt-remove-multiple_media-thumbnail">%s</a>','X' );
						$preview .= sprintf( '</div>');
					} 
				} else {
					$preview .= '';
		...
		echo SCPT_Markup::tag( 'div', array( 'id'=> $field['meta_key'], 'class' => 'scpt-multiple_media-preview' ), $preview );
		$multiple_media_button = sprintf( '<a href="#" class="button scpt-add-multiple_media">%s</a>',__( 'Add Images', 'slush-pro' ) );
		$multiple_media_button .= sprintf( '<a href="#" class="button scpt-clear-multiple_media">%s</a>',__( 'Clear Images', 'slush-pro' ) );       
		echo SCPT_Markup::tag( 'p', '', $multiple_media_button  );
		echo SCPT_Markup::tag( 'input', array(
			'type'  => 'hidden',
			'value' => $value,
			'name'  => $field['meta_key'],
	...
	/**
	 * Connects to another post type for one-to-one, one-to-many, or many-to-many relationships.
	 *
	...
	/**
	 * Prepares options for select elements, or prepare many checkbox or radio buttons.
	 *
	 * @uses SCPT_Markup::tag
	...
	public function prune_options( $options, $meta_key, $post_meta, $default_args = array(), $tag = 'option' ) {
		...
		// Allows developers to hook into this before the HTML is generated to override it.
	...
	/**
	 * Figures out which included field attributes are indended to be HTML attributes.
	 *
	...
	/**
	 * Saves values for custom meta boxes.
	 *
	 * @param int $post_id
	 * @return void
	 * @author Matthew Boynes
	...
	public function save_meta( $post_id ) {
		/* 
		 * Verifies if this is an auto save routine.
		 * If it is our form has not been submitted, so we dont want to do anything.
		 */
		if ( defined( 'DOING_AUTOSAVE' ) && DOING_AUTOSAVE )
			return;
		/*
		* Verifies this came from the our screen and with proper authorization,
	...
	/**
	 * Registers our datepicker CSS & JS.
	 *
	 * @return void
	 * @author Matthew Boynes
	 */
	...
	/**
	 * Adds the datepicker CSS to the doc head.
	 *
	 * @return void
	 * @author Matthew Boynes
	 */
	...
	/**
	 * Adds the datepicker JS to the doc head.
	 *
	 * @return void
	 * @author Matthew Boynes
	 */
