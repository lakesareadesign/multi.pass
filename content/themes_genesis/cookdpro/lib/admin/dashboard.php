...
add_action( 'after_switch_theme', 'cookd_dashboard_setup', 10 );
/**
 ...
 * @return  void
 */
function cookd_dashboard_setup() {
	if ( ! is_network_admin() ) {
		add_option( 'cookd_dashboard_redirect', true, '', 'no' );
	}
}

add_action( 'after_switch_theme', 'cookd_dashboard_redirect', 12 );
/**
 ...
 * @return  void
 */
function cookd_dashboard_redirect() {
	// Bail if we've already been redirected.
	if ( is_network_admin() || ! get_option( 'cookd_dashboard_redirect' ) ) {
		return;
	}

	// Make sure this doesn't go into a redirect loop.
	update_option( 'cookd_dashboard_redirect', false );

	wp_safe_redirect( admin_url( 'admin.php?page=cookd-dashboard' ) );
	exit;
}

add_action( 'switch_theme', 'cookd_dashboard_cleanup', 10 );
/**
 ...
 * @return  void
 */
function cookd_dashboard_cleanup() {
	delete_option( 'cookd_dashboard_redirect' );
}
...
add_action( 'admin_menu', 'cookd_dashboard_menu', 0 );
/**
 ...
 * @return  void
 */
function cookd_dashboard_menu() {
	add_menu_page(
		'Cook\'d Pro',
		'Cook\'d Pro',
		'edit_theme_options',
		'cookd-dashboard',
		'cookd_dashboard_page',
		cookd_get_svg_icon(),
		'58.997'
	);
}
 ...
 * @return  void
 */
function cookd_dashboard_page() {
	require_once COOKD_DIR . 'lib/admin/views/dashboard.php';
}

add_action( 'admin_enqueue_scripts', 'cookd_dashboard_scripts', 10 );
/**
 ...
 * @return  void
 */
function cookd_dashboard_scripts( $screen ) {
	if ( 'toplevel_page_cookd-dashboard' === $screen ) {
		wp_enqueue_style(
			'cookd-dashboard',
			COOKD_URI . 'css/dashboard.css',
			array(),
			CHILD_THEME_VERSION
		);
	}
